...
class Survey extends Base {
	...
	public function __construct( $plugin ) {
		...
		$this->api_url = "{$this->server}/wp-json/fusion10-lead/v1.0/";
		
		$this->notice_heading = '<h3>' . sprintf( __( 'Thanks for using \'<strong>%s</strong>\'', 'fusion10' ), $this->name ) . '</h3>';
		$this->notice_message = '<p> ' . __( 'We want to know what type of sites use this plugin. Users\' satisfaction is our first priority and we\'re continuously working on it. This is why we need some information so that we can improve it even more.<br />Help us with your site URL and a few basic information. It doesn\'t include your password or any secret data. Would you like to help us?', 'fusion10' ) . '</p>';
		$this->notice_button_text = __( 'Okay. Don\'t bother me again!', 'fusion10' );
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'fusion10-product-survey', plugins_url( 'assets/css/survey.css', __FILE__ ), [], $this->plugin['Version'] );
		wp_enqueue_script( 'fusion10-product-survey', plugins_url( 'assets/js/survey.js', __FILE__ ), [ 'jquery' ], $this->plugin['Version'], true );
	}
